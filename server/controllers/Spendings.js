...
const add_Spendings = async (req, res) => {
  ...
  const get_Spendings = async (req, res) => {
    ...
    try {
      const spendings = await spendingsSchema.find({ user: userid }).sort({ createdAt: -1 });
  ...
  const delete_Spending = async (req, res) => {
    ...
    try {
      const deletedSpending = await spendingsSchema.findOneAndDelete({ _id: id, user: userid });
      if (!deletedSpending) {
